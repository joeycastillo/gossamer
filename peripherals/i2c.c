...
void i2c_init(void) {
    ...
    SERCOM5->I2CM.CTRLA.bit.SWRST = 1;
    while (SERCOM5->I2CM.SYNCBUSY.bit.SWRST || SERCOM5->I2CM.SYNCBUSY.bit.ENABLE) {};
    ...
    SERCOM5->I2CM.CTRLA.bit.SWRST = 1;
    while (SERCOM5->I2CM.SYNCBUSY.bit.SWRST || SERCOM5->I2CM.CTRLA.bit.SWRST) {};

    SERCOM5->I2CM.CTRLA.reg = SERCOM_I2CM_CTRLA_SPEED(0) |        // 0 is standard/fast mode 100 & 400kHz
                                     SERCOM_I2CM_CTRLA_SDAHOLD(0) |      // Hold SDA low for 300-600ns
                                     SERCOM_I2CM_CTRLA_MODE_I2C_MASTER;  // work as master

    /* Enable smart mode */
    SERCOM5->I2CM.CTRLB.reg = SERCOM_I2CM_CTRLB_SMEN | SERCOM_I2CM_CTRLB_QCEN;
    ...
    */
    ...
    SERCOM5->I2CM.BAUD.bit.BAUD =
        clock_speed / (2 * baudrate) - 5 - (((clock_speed / 1000000) * rise_time) / (2 * 1000));
    while (SERCOM5->I2CM.SYNCBUSY.bit.SYSOP) {};

    /* Enable the SERCOM. */
    SERCOM5->I2CM.CTRLA.bit.ENABLE = 1;
    while (SERCOM5->I2CM.SYNCBUSY.bit.ENABLE) {};

    /* Put the bus into the idle state. */
    SERCOM5->I2CM.STATUS.bit.BUSSTATE = BUSSTATE_IDLE;
    while (SERCOM5->I2CM.SYNCBUSY.bit.SYSOP) {};
}
...
I2CResult i2c_write(uint8_t address, uint8_t* data, size_t len) {
    /* Before trying to write, check to see if the bus is busy, if it is,
       bail.
    */
    if (SERCOM5->I2CM.STATUS.bit.BUSSTATE == BUSSTATE_BUSY) {
        return I2C_RESULT_ERR_BUSSTATE;
    }

    /* Address + write flag. */
    SERCOM5->I2CM.ADDR.bit.ADDR = (address << 0x1ul) | 0;

    /* This can hang forever, so put a timeout on it. */
    size_t w = 0;
    for (; w < 100000; w++) {
        if (SERCOM5->I2CM.INTFLAG.bit.MB) {
            break;
        }
    }

    /* Check for loss of bus or NACK - in either case we can't continue. */
    if (SERCOM5->I2CM.STATUS.bit.BUSSTATE != BUSSTATE_OWNER) {
        return I2C_RESULT_ERR_BUSSTATE;
    }
    if (SERCOM5->I2CM.STATUS.bit.RXNACK) {
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        SERCOM5->I2CM.DATA.bit.DATA = data[i];

        while (!SERCOM5->I2CM.INTFLAG.bit.MB) {
            ...
            if (SERCOM5->I2CM.STATUS.bit.BUSERR) {
