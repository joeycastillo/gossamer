...
void app_setup(void) {
    ...
    if (HAL_GPIO_SWITCH_read()) {
        HAL_GPIO_A0_pmuxen(HAL_GPIO_PMUX_B);
        dac_init();
        dac_enable(0);
        tc_count16_set_cc(4, 0, ccval);
        tc_set_channel_polarity(4, 0, TC_CHANNEL_POLARITY_NORMAL);
        tc_set_channel_polarity(4, 1, TC_CHANNEL_POLARITY_INVERTED);
    } else {
        HAL_GPIO_A6_pmuxen(HAL_GPIO_PMUX_TC_TCC);
        HAL_GPIO_A7_pmuxen(HAL_GPIO_PMUX_TC_TCC);
        tc_count16_set_cc(4, 0, ccval * 50);
        tc_set_channel_polarity(4, 0, TC_CHANNEL_POLARITY_NORMAL);
        tc_set_channel_polarity(4, 1, TC_CHANNEL_POLARITY_INVERTED);
    }
...
bool app_loop(void) {
    if (HAL_GPIO_BUTTON_A_read() && !was_pressed) {
        was_pressed = true;
        ccval++;
        tc_count16_set_cc(4, 0, ccval);
    } else if (HAL_GPIO_BUTTON_B_read() && !was_pressed) {
        was_pressed = true;
        if (ccval > 1) ccval--;
        tc_count16_set_cc(4, 0, ccval);
    }

    if (HAL_GPIO_BUTTON_A_read() == 0 && HAL_GPIO_BUTTON_B_read() == 0) {
        was_pressed = false;
    }
