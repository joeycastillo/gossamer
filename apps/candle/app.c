...
void app_setup(void) {
    ...
    tc_set_wavegen(1, TC_WAVEGEN_NORMAL_PWM);
    ...
    tc_count8_set_cc(1, 0, 32);
    tc_count8_set_cc(1, 1, 0);
}
...
bool app_loop(void) {
    tcc_set_cc(0, 0, (flicker[i] / 6) >> MAX(maxLoops / loops - 1, 0));
    tcc_set_cc(0, 1, (flicker[i] / 2) >> MAX(maxLoops / loops - 1, 0));
    ++i;
    tc_count8_set_cc(1, 0, (flicker[i] / 8) >> MAX(maxLoops / loops - 1, 0));
    i = i % (sizeof(flicker) - 1);
    delay_ms(50);

    if (HAL_GPIO_BUTTON1_read() == false) loops = 0;
    if (HAL_GPIO_BUTTON2_read() == false) loops = maxLoops;
    ...
    if (--loops <= 0) {
        ...
        HAL_GPIO_WARM_WHITE_pmuxdis();
        HAL_GPIO_COOL_WHITE_pmuxdis();
        HAL_GPIO_RED1_set();
        HAL_GPIO_RED2_set();
        HAL_GPIO_RED3_set();
        HAL_GPIO_GREEN1_set();
