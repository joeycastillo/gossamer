...
uint8_t _colstart = 0,  ///< Some displays need this changed to offset
        ...
        _tabcolor = 0;  ///< The color of the tab on the screen protector

// SCREEN INITIALIZATION ***************************************************

// Rather than a bazillion writecommand() and writedata() calls, screen
// initialization commands and arguments are organized in these tables
// stored in Flash.  The table may look bulky, but that's mostly the
...
void st7735_init_b(void) {
    st77xx_display_init(Bcmd);
}

void st7735_init_r(uint8_t options) {
    st77xx_display_init(Rcmd1);
    if (options == INITR_GREENTAB) {
        st77xx_display_init(Rcmd2green);
        _colstart = 2;
        _rowstart = 1;
    } else if ((options == INITR_144GREENTAB) || (options == INITR_HALLOWING)) {
        gfx_raw_height = 128;
        gfx_raw_width = 128;
        st77xx_display_init(Rcmd2green144);
        _colstart = 2;
        _rowstart = 3; // For default rotation 0
    } else if (options == INITR_MINI160x80) {
        gfx_raw_height = 80;
        gfx_raw_width = 160;
        st77xx_display_init(Rcmd2green160x80);
        _colstart = 24;
        _rowstart = 0;
    } else {
        // colstart, rowstart left at default '0' values
        st77xx_display_init(Rcmd2red);
    }
    st77xx_display_init(Rcmd3);
    ...
    if (options == INITR_HALLOWING) {
        ...
        _tabcolor = INITR_144GREENTAB;
        // gfx_set_rotation(180);
    } else {
        _tabcolor = options;
        gfx_set_rotation(0);
    }
